...
class DANBlank(Probe):
    """Blank DAN Probe, to be filled up by prompts later on"""
...
class DAN_Jailbreak(DANBlank, Probe):
    """DAN Jailbreak"""
...
class ChatGPT_Developer_Mode_v2(DANBlank, Probe):
...
class ChatGPT_Developer_Mode_RANTI(DANBlank, Probe):
...
class ChatGPT_Image_Markdown(DANBlank, Probe):
...
class Ablation_Dan_11_0(DANBlank, Probe):
...
class AutoDANProbe(DANBlank, Probe):
